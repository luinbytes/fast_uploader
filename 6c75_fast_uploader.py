...
CONFIG_FILE = 'fast_uploader.json'
...
class ConfigGUI(tk.Tk):
    ...
    def create_widgets(self):
        ...
        style.configure('Treeview.Heading', background=THEME['tree_heading'], foreground=THEME['btn_fg'])
        ...
        self.toast_pos_combo = ttk.Combobox(toast_frame, textvariable=self.toast_pos_var, values=['Top Left', 'Top Right', 'Bottom Left', 'Bottom Right'], state='readonly')
        ...
        self.toast_duration_entry = ttk.Entry(duration_frame, textvariable=self.toast_duration_var)
        ...
        refresh_button = ttk.Button(button_frame, text='🔄 Refresh Stats', command=self.populate_folder_stats)
        refresh_button.pack(side='right', expand=True, padx=5)

        credits_label = ttk.Label(self, text="6c75/luinbytes | github.com/luinbytes | Helped by Gemini CLI", background=THEME['bg'], foreground=THEME['fg'], font=('Arial', 8))
    ...
    def populate_folder_stats(self):
        ...
        folders = [d for d in os.listdir(current_dir) if os.path.isdir(os.path.join(current_dir, d)) and not d.startswith('$')]
        
        folder_data = []
        for folder_name in folders:
            folder_path = os.path.join(current_dir, folder_name)
            video_files = [f for ext in VIDEO_EXTENSIONS for f in glob.glob(os.path.join(folder_path, '**', ext), recursive=True)]

            file_count = len(video_files)
            total_size = sum(os.path.getsize(f) for f in video_files)
            avg_size = total_size / file_count if file_count > 0 else 0
            extensions = [os.path.splitext(f)[1] for f in video_files if os.path.splitext(f)[1]]
            top_ext = max(set(extensions), key=extensions.count) if extensions else 'N/A'
...
class ToastGUI(tk.Toplevel):
    def __init__(self, parent, clip_info, uploader, position, duration_s):
        ...
        main_frame.pack()
        ttk.Label(main_frame, text='6c75 Fast Uploader 🚀', font=('Arial', 12, 'bold')).pack(pady=(0, 10))
    ...
    def set_position(self, position):
        ...
        if position == 'Top Left': x, y = 10, 10
        elif position == 'Top Right': x, y = screen_width - width - 10, 10
        elif position == 'Bottom Left': x, y = 10, screen_height - height - 40
        else: x, y = screen_width - width - 10, screen_height - height - 40
...
def main():
    if not os.path.exists(CONFIG_FILE):
        with open(CONFIG_FILE, 'w') as f:
            json.dump({'sharex_host': 'ezhost', 'toast_position': 'Bottom Right', 'toast_duration': '5', 'uploaded_files': []}, f, indent=4)
    
    with open(CONFIG_FILE, 'r') as f:
        data = json.load(f)

    current_dir = os.getcwd()
    all_folders = [d for d in os.listdir(current_dir) if os.path.isdir(os.path.join(current_dir, d)) and not d.startswith('$')]
    video_files = [f for folder in all_folders for ext in VIDEO_EXTENSIONS for f in glob.glob(os.path.join(folder, '**', ext), recursive=True)]
    ...
    if absolute_latest_video_name in uploaded_files:
        ...
        try:
            ...
            root = tk.Tk()
            root.withdraw()
            clip_info = {'name': absolute_latest_video_name, 'size': ConfigGUI.format_size(None, os.path.getsize(absolute_latest_video))}
            duration = float(data.get('toast_duration', 5))
            toast = ToastGUI(root, clip_info, sharex_host, data.get('toast_position', 'Bottom Right'), duration)
            toast.mainloop()
